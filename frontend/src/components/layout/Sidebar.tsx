...
export function Sidebar() {
    ...
    return (
        <>
            ...
            <button
                className="md:hidden fixed top-4 left-4 z-50 p-2 rounded bg-white shadow border"
            ...
            {/* Sidebar responsiva */}
            <aside
                className={`bg-white border-r shadow-sm flex flex-col fixed top-0 left-0 h-full w-64 z-40 transition-transform duration-300 md:static md:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}
            >
                ...
                <button
                    className="md:hidden m-4 px-4 py-2 rounded bg-indigo-600 text-white font-semibold"
